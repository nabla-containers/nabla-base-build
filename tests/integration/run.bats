...
function nabla_run() {
	run ${TOPLEVEL}/solo5/ukvm/ukvm-bin --net=tap_nabla_test "$@"
...
@test "test raw node version" {
	nabla_run --disk="${BATS_TMPDIR}"/dummy ${TOPLEVEL}/node-base/node.nabla -- '{"cmdline":"node --version"}'
	[[ "$output" == *"v4.3.0"* ]]
	[ "$status" -eq 0 ]
}
...
@test "test raw python version" {
	nabla_run --disk="${BATS_TMPDIR}"/dummy ${TOPLEVEL}/python3-base/python3.nabla -- '{"cmdline":"python --version"}'
	[[ "$output" == *"Python 3.5.2"* ]]
	[ "$status" -eq 0 ]
}

@test "test raw redis version" {
	nabla_run --disk="${BATS_TMPDIR}"/dummy ${TOPLEVEL}/redis-base/redis.nabla -- '{"cmdline":"redis --version"}'
	[[ "$output" == *"Redis server v=3.0.6"* ]]
	[ "$status" -eq 0 ]
}

@test "test raw nginx version" {
	nabla_run --disk="${BATS_TMPDIR}"/dummy ${TOPLEVEL}/nginx-base/nginx.nabla -- '{"cmdline":"nginx -v"}'
